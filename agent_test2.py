...
def load_agent_prompt(specific_obj=None):
    ...
    if specific_obj:
        ...
        # Erweiterung für Vision-Analyse
        vision_instr = "\n\nDEEP ANALYSIS MODUS AKTIV: Du siehst das Bild dieses Objekts. Beziehe dich in deiner Antwort explizit auf visuelle Details (Farben, Komposition, dargestellte Personen/Gegenstände)."
        return f"{base}\n\nAKTUELLER FOKUS AUF DIESES OBJEKT:\n{json.dumps(clean_obj, ensure_ascii=False)}{vision_instr}"
    return base
...
with col_chat:
    if st.session_state.active_obj:
        ...
        st.markdown(f'<div class="fokus-banner"><strong>👁️ DEEP ANALYSIS AKTIV:</strong> {obj["title"]}</div>', unsafe_allow_html=True)
    ...
    if prompt := st.chat_input("Frage zum Objekt stellen..."):
        ...
        with chat_box:
            ...
            with st.chat_message("assistant"):
                ...
                # Multimodale Nachricht vorbereiten
                content_parts = [types.Part(text=prompt)]
                
                # Wenn Fokus aktiv, Bild mitsenden
                if st.session_state.active_obj:
...
with col_right:
    if st.session_state.active_obj:
        ...
        st.markdown(f'<div class="compact-detail-card"><div class="detail-img-box"><img src="{img_url}"></div><div class="meta-line"><b>Inv. Nr.:</b> {obj.get("inventory_number", "k.A.")}</div><div style="font-weight: bold; margin-bottom: 5px;">{obj["title"]}</div><div class="desc-scroll">{obj.get("description", "Keine Beschreibung vorhanden.")}</div></div>', unsafe_allow_html=True)
        if obj.get('id'):
